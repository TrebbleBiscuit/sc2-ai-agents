...
def main():
    possible_ai_builds = [
        AIBuild.Rush, AIBuild.Timing, AIBuild.Power, AIBuild.Macro, AIBuild.Air,
        # AIBuild.RandomBuild
    ]
    for i in range(0, 1):
        opponent_race = [Race.Zerg, Race.Terran, Race.Protoss][random.randrange(0,3)]
        difficulty = Difficulty.CheatMoney
        global build  # yeah yeah i know i just wanna log this for now ok
        build = possible_ai_builds[random.randrange(0, len(possible_ai_builds))]
        print(f"opponent_race is {opponent_race}, difficulty is {difficulty}, build is {build}")
        sc2.run_game(
            sc2.maps.get("CatalystLE"),  # select map
            # [Bot(Race.Terran, DeckardBot()), Computer(Race.Zerg, Difficulty.CheatMoney, AIBuild.Rush)],  # select opponent race and difficulty
            [Bot(Race.Terran, DeckardBot()), Computer(opponent_race, difficulty, build)],
...
# SCV should build before the 1st refinery starts
# Make sure reaper actually builds
# Put more production buildings in main (esp factories and starports)
# Less stuff clogging up the natural
...
# Tanks should target robo units / other tanks / ultras, etc
"""
...
class DeckardBot(sc2.BotAI):  # Do things here before the game starts
    ...
    async def on_start(self):
        ...
        self.army_group = utils.ArmyGroup(self)
    ...
    async def on_step(self, iteration):
        ...
        if iteration == 0:  # All the stuff to do exactly once when the game starts
            ...
            assert len(self.depot_placement_positions) == 2
            # set these positions for ramp depots to be used later
            self.depot_r1 = self.depot_placement_positions.pop()
            self.depot_r2 = self.depot_placement_positions.pop()
        ...
        if (
            self.supply_workers < 10 and not self.townhalls
            or (
                self.townhalls.amount + 2 < len(self.opponent_info['expansions'])  # enemy has 3+ more townhalls than we do
                and self.get_enemy_army_value('value') > self.get_army_value('value') * 3  # scouted enemy army value is triple ours
                and self.get_army_value('value') < 800  # army is small
                )
        ):
            if not self.willsurrender:
                ...
                self.chatty = False  # i don't really like this at the end
                if self.chatty:
        ...
        if iteration % 4 == 0:  # Do less frequently
            await self.distribute_workers(resource_ratio=2) # will distribute workers between bases, minerals, and gas geysers semi-efficiently
        ...
        # await self.cancel_buildings()               # TODO: cancel buildings in production
    ...
    async def army_movement(self, iteration):
        ...
        self.army_group.add_to(self.forces)
        army_group_units = self.army_group.get_units()
        ...
        #
            ...
            if self.enemy_units.closer_than(60, self.start_location):  # delay attack if there are enemy units nearby
                ...
                print("delaying attack due to nearby threat")
                return
            # TODO
        ...
        elif self.army_mood == "ATTACK" and self.time < self.attack_start_time + self.get_attack_length():
        ##### continue attack
            if not self.announced_efficiency and self.attack_length + 50 < self.get_attack_length():
                ...
                print("we're trading very efficiently here")
                self.announced_efficiency = True
            for u in self.forces:
                nearby_threats = self.enemy_units.closer_than(35, u).exclude_type({LARVA, OBSERVER})  # enemy units nearby
                if nearby_threats:
                    if iteration % 5 == 0:  # TODO: do this by time_since instead?
                        u.attack(nearby_threats.closest_to(u).position)  # attack towards the nearest threat
                elif u.is_idle:
                    u.attack(self.get_base_target())  # otherwise if idle attack towards normal target
            if iteration % 5 == 0:
        ...
        # elif round(self.time) == self.attack_start_time + round(self.get_attack_length()) and self.army_mood != "DEFEND":  # what the hell is this garbage?
            print("beginning defense")
            self.army_mood = "DEFEND"
        ...
        ##### continue defense
            if iteration % 6 == 0:
                ...
                for unit in self.forces:
                    hostiles = self.enemy_units.closer_than(20, unit).exclude_type({LARVA})  # hostiles are close to an individual unit
                    ...
                    elif len(self.forces) < 5:  # few forces exist
                        continue  # stay where you are (probably near rax)
    ...
    async def manage_spending(self):
        ...
        """
        if await self.upgrade_command_center() == "cant_afford":
            return  # always save money to do this first
        await self.train_workers()
        await self.expand()
        await self.build_depots()
        await self.build_addons()  # must be before unit training so that buildings build addons first
        if (  # If NONE of these conditions are true, SAVE MONEY to expand.
            self.townhalls.ready.amount >= self.get_ideal_building_count("COMMANDCENTER")  # don't want to expand
            or self.already_pending(UnitTypeId.COMMANDCENTER)  # already queued command center
            or self.enemy_units and self.townhalls.amount == 1  # enemy units nearby and we're on 1base
            or self.get_enemy_army_value('value') > self.get_army_value('value') * 1.1  # scouted enemy army value is much greater than ours
        ):
        ...
        if self.vespene < 200 or self.minerals > 150 or self.time < 120:
    ...
    async def handle_scouted_information(self):
        ...
        if not self.scouted_pool:
            for pool in self.enemy_structures.of_type(SPAWNINGPOOL):
                ...
                print("found the enemy spawning pool!")
                ...
                if started_at < 36 and not self.beingcheesed:  # TODO: tune this, the pool will already be done when the SCV gets there so...
                    self.beingcheesed = True
                    self.attack_norush_period += 60  # delay timing attack
                    await self.chat_send("nice pool lol")
        # now for units
        if not self.beingcheesed:  # only checks if we didn't already see the pool
            if self.time < 120 and self.enemy_units.of_type(ZERGLING):  # under 2:00, scout lings
                self.beingcheesed = True
                print("ling flood inc")
                self.attack_norush_period += 80  # delay timing attack
    ...
    async def ability_interference_matrix(self):
        for raven in self.units(UnitTypeId.RAVEN):
            targets = self.enemy_units.of_type({COLOSSUS, MOTHERSHIP, SIEGETANKSIEGED, BATTLECRUISER, VIPER, INFESTOR, ULTRALISK}).closer_than(16, raven)
            if targets:
    ...
    async def build_production(self):
        """
        if self.workers.gathering:  # workers on their way to get resources
            buildingscv = self.workers.gathering.random
        elif self.workers.collecting:  # workers actively collecting OR on their way to get resources
            buildingscv = self.workers.collecting.random
        else:
            return
        """
        ...
        if (  # 1st rax goes in the wall
            ...
            self.workers.gathering.closest_to(self.barracks_placement_position).build(UnitTypeId.BARRACKS, self.barracks_placement_position)
            # await self.build(UnitTypeId.BARRACKS, self.barracks_placement_position)
        elif (  # other raxes go wherever
            self.can_afford(UnitTypeId.BARRACKS)
            and self.tech_requirement_progress(UnitTypeId.BARRACKS) == 1
    ...
    async def build_addons(self):
        ...
        # TODO: change this dependong on opponent race and unit composition
        ...
        for rax in self.structures(UnitTypeId.BARRACKS).ready.idle:
            if (  # conditions to build an add-on:
                not rax.has_add_on
                # and not self.enemy_units.closer_than(8, rax.position)  # build units instead if there are nearby hostiles
                # excluding this for now bc it wont build addons to finish a wall. 
                # TODO: if rax is at self.main_base_ramp.barracks_correct_placement
            ):
                addon_position: Point2 = rax.position + Point2((2.5, -0.5))
                # TODO: ONLY LIFT IF THERE ARE NO ENEMY UNITS NEARBY
                if (
                    not await self.can_place(UnitTypeId.SUPPLYDEPOT, addon_position)  # can't place an addon here
                    and not rax.position.distance_to(self.main_base_ramp.barracks_correct_placement) > 2  # rax isn't the one in the wall
                ):
                    rax(AbilityId.LIFT)  # if an addon won't fit, lift
    ...
    async def train_workers(self):
        # TODO: if command center (not orbital) and we have a rax, don't build workers (wait for orbital instead)
        for cc in self.townhalls:
            if (
        ...
        for rax in self.structures(UnitTypeId.BARRACKS).ready:
            if rax.is_idle:
                if rax.has_techlab:
                    ...
                    self.time > 210  # after 3:30
                    and len(self.opponent_info["expansions"]) < 3  # haven't scouted opponent's third
                    and not self.units.of_type(REAPER)  # don't already have a reaper
                ):
                    if self.can_afford(UnitTypeId.REAPER): rax.train(UnitTypeId.REAPER)
                elif rax.has_reactor or self.time < 60:
                    if self.can_afford(UnitTypeId.MARINE): rax.train(UnitTypeId.MARINE)
                else:
                    pass  # won't train from rax with no addons
    ...
    async def train_from_factory(self):
        for fac in self.structures(UnitTypeId.FACTORY).ready.idle:
            ...
            elif self.can_afford(UnitTypeId.HELLION) and self.beingcheesed and self.time < 240:
    ...
    def get_enemy_army_value(self, f='res'):
        assert f in ['res', 'value']  # 'resources' (min, gas) or 'value' (int)
        value_minerals = 0
        value_gas = 0
        ...
        if f == 'res':
            ...
            return value_minerals + (value_gas * self.mineral_gas_ratio)

    def get_army_value(self, f='res'):
        assert f in ['res', 'value'] 
        value_minerals = 0
        value_gas = 0
        for unit in self.units:
            if unit.type_id not in {SCV, MULE} and not unit.is_structure:
    ...
    def get_attack_length(self, reset=False):
        ...
        value_calc = self.attack_length + (self.total_value_killed - self.total_value_lost)/80
        return value_calc
    ...
    def get_ideal_building_count(self, building):
        ...
        if building == "COMMANDCENTER":
            if self.beingcheesed: adjusted_time -= 70  # take natural 70 seconds later 
            elif self.be_aggressive and th >= 2: adjusted_time -= 60  # take 3rd 60 seconds later

            if adjusted_time < 100: ccc = 1    # 1:40
            elif adjusted_time < 330: ccc = 2  # 5:30
            elif adjusted_time < 560: ccc = 3  # 9:20
            elif adjusted_time < 690: ccc = 4  # 11:30
            elif adjusted_time < 840: ccc = 5  # 14:00
            else: ccc = 6

            if self.minerals > 1200: ccc += 1
            return ccc
        
        elif building == "BARRACKS":
            if th == 1: ib = 1
            elif self.townhalls.ready.amount == 2: ib = 2  # 2 finished CCs
            elif th == 2: ib = 1  # natural started
            elif th == 3: ib = 4
            elif th == 4: ib = 7
            elif th == 5: ib = 10
            else: ib = 15

            if self.beingcheesed and self.time > 100 and self.townhalls.ready.amount == 1: ib += 1
            if self.townhalls.ready.amount >= 2 and self.be_aggressive: ib += 2  # 2 more rax on 2+ bases
            if self.minerals > 700:
                if self.time < 360: ib += 1
                else: ib += 2
            return ib
    ...
    def get_base_target(self):
        ...
        # else:
        enemy_expo_pos_list = [p for p in self.opponent_info["expansions"]]
        min_dist = 99
        closest_enemy_base = None
        for expo in self.opponent_info["expansions"]:
            this_dist = expo.distance_to(self.townhalls.center)  # closest enemy base to our territory
            if this_dist < min_dist:
                min_dist = this_dist
                closest_enemy_base = expo
    ...
    def showdebuginfo(self, iteration):
        ...
        their_av = "Hostile Army Value:  " + str(self.get_enemy_army_value())
        self._client.debug_text_screen(text=their_av, pos=Point2((0, 0.21)), color=None, size=8)

        our_av = "Friendly Army Value: " + str(self.get_army_value())
        self._client.debug_text_screen(text=our_av, pos=Point2((0, 0.22)), color=None, size=8)
        ...
        # TODO: add this thing that lets you surrender early
        # TODO: also add one that lets you win if that's an option?
        # for e in self.state.chat:
        #     # print(e.message)
        #     if e.message == "debug gg":
        #         self.surrendernow = True
        #         await self._client.leave()
            
        # if len(self.state.chat) > 0:
        #     print(self.state.chat[0].message)
    ...
    def on_end(self, result):  # Do things here after the game ends
        # TODO: compile_stats() function or smth
        # Length of game
        # Matchup
        self.my_stats['result'] = result
        self.my_stats['lost_and_killed'] = self.get_resources_lost(typ='t')
        self.my_stats['game_length'] = round(self.time, 2)
        try:
            self.my_stats['ai_build'] = build
        except NameError:
            pass
        print(self.my_stats)
        print("self.opponent_info['army_units_scouted'] is: ")
        print(self.opponent_info['army_units_scouted'])
        print("Game ended.")
        print(f"Result was {result}")
        if self.make_log:
            self.logfile.write(f"{self.my_stats}\n")
            if self.surrendernow:
                self.logfile.write("Surrendered. Can't win 'em all. \n")
